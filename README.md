...
**Status:** Pre-release / under active development. All core tabs shipped end-to-end: Traffic (with the Phase 8 world heatmap MAP sub-view + top-3 destinations per country on hover), Firewall (ALLOW/BLOCK/DEFAULT pills + master toggle + activity strip), Alerts (master-detail + OS toasts + spoof detection), Scanner (LAN device discovery + manual labels), and as of **Phase 13.2** — the **Settings** tab gets its first interactive sub-phase. Settings 13.1 had opened with three read-only sections (Data Storage, Maintenance, About). 13.2 adds two sections that actually mutate daemon state from the UI: **Recording** (one toggle: `FilterSelfTraffic`) and **Hostname Resolution** (three toggles: `EnablePreload`, `EnableReverseDnsFallback`, `EnableSniCapture`). The architecture established in 13.2 — per-section state singletons mirroring `IFirewallEnforcementState`, a new `settings_overrides` SQLite table for persistence, a `SettingsOverridesService : IHostedService` that loads overrides at startup before any consumer reads, three new RPCs (`GetSettings` + two atomic `Set*` handlers), and two new chain-event kinds with deterministic JSON payload encoders — is the template that 13.3 / 13.4 / 13.5 will each reuse. The gRPC RPC surface grows from 19 to 22. `FilterSelfTraffic` and `EnableReverseDnsFallback` toggle live (read per flow event / per cache miss); `EnablePreload` and `EnableSniCapture` consumers snapshot at `StartAsync`, so the UI renders `(takes effect on next daemon start)` beside those pills. Set RPCs follow the existing `SetLanDeviceLabel` / `TriggerScan` soft-failure precedent — persistence failures return a structured `success=false` rather than throwing. See [ADR 010](docs/decisions/010-runtime-mutable-settings.md) for the full rationale. **1276 tests** pass deterministically (was 1231 after 13.1.2; +45 in 13.2 across state singletons, payload encoders, the new SQLite store, the hosted service, the three RPC handlers, and the toggle commands on the Settings VM). Next up: 13.3 (Alerts section) and onward through the Settings sub-phases. See [`docs/phases.md`](docs/phases.md) for the current state, lessons learned, and the full roadmap.
