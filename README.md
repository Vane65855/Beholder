...
**Status:** Pre-release / under active development. All core tabs shipped end-to-end: Traffic (with the Phase 8 world heatmap MAP sub-view + top-3 destinations per country on hover), Firewall (ALLOW/BLOCK/DEFAULT pills + master toggle + activity strip), Alerts (master-detail + OS toasts + spoof detection), Scanner (LAN device discovery + manual labels), and as of **Phase 13.3** — the **Settings** tab gains its second interactive sub-phase. 13.2 added Recording + Hostname Resolution sections; 13.3 adds an **Alerts** section with three master kill-switches for the alert detector pipeline: `EnableNewProcessDetection` (live, per flow event), `EnableHashChangeDetection` (live, per periodic tick), `EnableChainIntegrityMonitor` (live, per periodic tick). All three are live — no restart-required captions. Numeric thresholds (`BinaryHashCheckIntervalMinutes`, `ChainVerifyIntervalMinutes`, `MaxFileHashTimeoutSeconds`) stay JSON-only matching the Phase 13.2 precedent for advanced tuning. **Behavior change (security fix):** the mandatory startup chain verify can no longer be skipped via the UI toggle — `ChainIntegrityMonitor.StartAsync` always runs the startup verify; the `EnableChainIntegrityMonitor` toggle now gates only the *periodic* loop. Previously, a corrupt chain could be silently hidden by toggling the monitor off before restart. RPC surface 22 → 23. New `EventKind.AlertSettingsChanged = 13` + `AlertSettingsPayloadEncoder` round out the chain-audit story. See [ADR 010 addendum](docs/decisions/010-runtime-mutable-settings.md) for the mandatory-startup-verify rationale. **1294 tests** pass deterministically (was 1276 after 13.2; +18 in 13.3 across state-singleton, payload-encoder, RPC-handler, and VM-toggle tests, plus one modified `ChainIntegrityMonitorTests` test enforcing the new contract). Next up: 13.4 (Scanner section) and onward through the Settings sub-phases. See [`docs/phases.md`](docs/phases.md) for the current state, lessons learned, and the full roadmap.
