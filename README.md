...
**Status:** Pre-release / under active development. All core tabs shipped end-to-end: Traffic (with the Phase 8 world heatmap MAP sub-view + top-3 destinations per country on hover), Firewall (ALLOW/BLOCK/DEFAULT pills + master toggle + activity strip), Alerts (master-detail + OS toasts + spoof detection), and as of **Phase 9.4** — the **Scanner** tab. The Windows LAN scanner ships end-to-end: ARP cache walk + parallel SendARP for device discovery (9.2 / 9.2.1), DNS-Based Service Discovery browsing as the primary hostname-resolution path (9.2.6), plus per-IP mDNS-PTR + NetBIOS fallback (9.2.5). Phase 9.3 added the IPC surface; 9.4 the master-detail UI. **Phase 9.5** adds the two GlassWire-parity hostname features that were the most-common smoke-test gap: a `RouterDnsHostnameProbe` that picks up DHCP-table hostnames from routers that mirror DHCP into their LAN DNS resolver (joins the per-IP ladder after mDNS-PTR + NetBIOS), and a manual rename UX in the Scanner tab so any device can be given a user-supplied display name. Labels persist in SQLite (NOT chain-audited — cosmetic UI state, same category as Alert read-state), survive scanner re-observations, and propagate via a new `LanDeviceLabelChanged` stream event for live cross-instance refresh. The gRPC RPC surface grows from 17 to 18 (`SetLanDeviceLabel`). All five required UI states implemented and tested per `UI_QUALITY_STANDARDS.md`. Per [ADR 009](docs/decisions/009-scanner-as-lan-device-discovery.md). **1130 tests** pass deterministically. Next up: 9.6 (Traffic-tab cross-link from a Scanner device IP + verification pass), then Phase 10 (Uplink client). See [`docs/phases.md`](docs/phases.md) for the current state, lessons learned, and the full roadmap.
