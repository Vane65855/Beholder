...
**Status:** Pre-release / under active development. All core tabs shipped end-to-end: Traffic (with the Phase 8 world heatmap MAP sub-view + top-3 destinations per country on hover), Firewall (ALLOW/BLOCK/DEFAULT pills + master toggle + activity strip), Alerts (master-detail + OS toasts + spoof detection), and as of **Phase 9.4** — the **Scanner** tab. The Windows LAN scanner ships end-to-end: ARP cache walk + parallel SendARP for device discovery (9.2 / 9.2.1), DNS-Based Service Discovery browsing as the primary hostname-resolution path (9.2.6), plus per-IP mDNS-PTR + NetBIOS fallback (9.2.5). Phase 9.3 added the IPC surface (`ListLanDevices` and `TriggerScan` RPCs plus `LanDeviceFirstSeen` / `LanDeviceMacChanged` stream events; gRPC surface grew from 15 to 17 RPCs). **Phase 9.4** closes the loop with the Scanner tab UI: master-detail layout (virtualized device list on the left, per-device detail pane on the right), live updates via the stream events, "Scan now" button driving `TriggerScan` with the structured response surfaced as a transient banner. All five required UI states from `UI_QUALITY_STANDARDS.md` (loading / empty / populated / error / extreme) are implemented and tested. Per [ADR 009](docs/decisions/009-scanner-as-lan-device-discovery.md). **1099 tests** pass deterministically. Next up: 9.5 (Traffic-tab cross-link from a device IP + manual device labelling — to match GlassWire's user-rename UX), 9.6 (verification), then Phase 10 (Uplink client). See [`docs/phases.md`](docs/phases.md) for the current state, lessons learned, and the full roadmap.
