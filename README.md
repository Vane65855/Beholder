...
**Status:** Pre-release / under active development. All core tabs shipped end-to-end: Traffic (with the Phase 8 world heatmap MAP sub-view + top-3 destinations per country on hover), Firewall (ALLOW/BLOCK/DEFAULT pills + master toggle + activity strip), Alerts (master-detail + OS toasts + spoof detection), Scanner (LAN device discovery + manual labels), and as of **Phase 13.1** — the **Settings** tab foundation. Settings 13.1 opens with three read-only sections: Data Storage (per-table SQLite row counts + database file size, backed by the new `GetStorageStats` RPC), Maintenance ("Verify chain integrity now" button with last-verified timestamp + "Open data folder" button), and About (version + license + third-party attributions). The new `IChainStatusCache` singleton is shared between the periodic `ChainIntegrityMonitor` and the user-triggered `VerifyChain` RPC so both writers update the same Settings-tab-visible snapshot. Interactive sections (Storage retention preset, Recording, Hostname Resolution, Alerts, Scanner) ship one-per-sub-phase in 13.2–13.6, each adding its own runtime-mutable state singleton per the `FirewallEnforcementState` precedent. The gRPC RPC surface grows from 18 to 19 (`GetStorageStats`). All required UI states implemented and tested per `UI_QUALITY_STANDARDS.md`. **Phase 13.1.1** added the visual polish pass — pretty-named + retention-annotated tables in cascade order, per-row proportional bars + stacked total breakdown + cascade sparkline, chain-integrity pill badge with smooth color transition, MOTD-style status strip with live uptime, ASCII-eye brand mark, clickable URLs + copy-to-clipboard buttons, sticky section headers on scroll, left-edge accent stripes. **Phase 13.1.2** dropped the originally-included responsive 2-column layout (Data Storage was rendering at half-width due to a missing `Grid.ColumnSpan` AND the wide-mode style overrides didn't activate in Avalonia 12) — Settings is now a clean single full-width column, with per-row proportional bars that flex genuinely with window width. **1231 tests** pass deterministically. Next up: 13.2 (Recording + Hostname Resolution sections), then onward through the Settings sub-phases. See [`docs/phases.md`](docs/phases.md) for the current state, lessons learned, and the full roadmap.
