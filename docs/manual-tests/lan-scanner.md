...
- **K (hostname count) less than N (device count) is normal, but K=0 is uncommon on modern LANs (Phase 9.2.6+)**: expected. Not every LAN device advertises a hostname-resolvable service — modern phones with MAC randomization frequently advertise nothing; minimal home routers respond to nothing; many Windows installs have NetBIOS-over-TCP/IP disabled (`TcpipNetbiosOptions=0` on the active NIC, the modern default). However, Phase 9.2.6 added DNS-Based Service Discovery (RFC 6763) as the primary hostname-resolution path, which queries for 12 well-known service-types (`_airplay`, `_googlecast`, `_smb`, `_workstation`, `_printer`, `_ipp`, `_raop`, `_hap`, `_spotify-connect`, `_hue`, `_ssh`, `_companion-link`) — devices that advertise *any* of these (which covers most modern Apple / Google / IoT / printer / NAS hardware) now resolve. If K is still 0 after 9.2.6, the LAN genuinely has no SD-advertising devices and no NetBIOS responders. Verify with `dns-sd -B _services._dns-sd._udp` on macOS or `avahi-browse -a` on Linux — if those also return empty, your LAN has no mDNS chatter and Beholder's K=0 is correct behaviour. Useful hits typically include Apple devices (`_airplay`, `_companion-link`, `_raop`), Chromecasts / Google Home (`_googlecast`), Sonos speakers (`_airplay`, `_raop`), Philips Hue bridges (`_hue`), network printers (`_ipp`, `_printer`), NAS (`_smb`), Linux/Avahi machines (`_workstation`, `_ssh`), and the local Windows machine (NetBIOS, when enabled).
