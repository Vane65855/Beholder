...
- **Phase 13.1.1 / 13.1.2:** The Settings tab introduced a responsive 2-column layout that toggled a `wide` class on the root `UserControl` via a `Bounds.Width`-watching `ArrangeOverride` and consumed it via descendant style selectors (`UserControl.wide Border#MaintenanceCard`). In real runtime the layout never activated — the descendant selectors didn't re-evaluate when the class toggled, and Data Storage (which had no explicit `Grid.ColumnSpan` style) defaulted to span 1 of a `*,*` grid, leaving the right half of the viewport empty. The fix was to drop the responsive 2-column layout entirely and ship the Settings tab as a single full-width column. **Lesson: pure-XAML responsive layouts driven by programmatically-toggled `Classes` on a root control should not be used in Avalonia 12 without a verified runtime test. When in doubt, prefer always-on layouts that don't depend on class-state-driven style re-evaluation.**

Each issue was caused by the same underlying pattern: choosing the path of least resistance (fixed pixels, StackPanel over Grid, hardcoded values, untested style selectors) and verifying only at the developer's current resolution with the current data set.
