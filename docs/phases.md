...
**Current checkpoint:** Phase 9.2.6 (Scanner hostname resolution — DNS-Based Service Discovery per RFC 6763. One-shot batched browse per scan sends multicast PTR queries for 12 well-known service-type names — `_airplay`, `_googlecast`, `_smb`, `_workstation`, `_printer`, `_ipp`, `_raop`, `_hap`, `_spotify-connect`, `_hue`, `_ssh`, `_companion-link` (all `._tcp.local`) — from one `UdpClient`; a 3-second receive loop parses PTR + SRV + A records across answers + authority + additional sections and correlates them per-instance to extract IP → hostname pairs. Replaces 9.2.5's per-IP reverse-PTR ladder as the **primary** hostname-resolution path; the 9.2.5 ladder is now demoted to fallback for IPs the SD browse didn't cover. Same trust posture as 9.2.5: pure managed `UdpClient`, no new P/Invoke, defensive bounds-checked parsers mirroring `TlsClientHelloParser` per ADR 006, same `EnableHostnameResolution` kill-switch. DNS name compression / pointer-following code was extracted to a shared `DnsNameDecoder` helper so both `MdnsPacketParser` (9.2.5) and `MdnsServiceDiscoveryParser` (9.2.6) reuse the same loop-guarded implementation per DRY.)
**Test count:** 1029
