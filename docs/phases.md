...
**Current checkpoint:** Phase 9.2 (Scanner ARP probe + scheduler — Windows ARP probe via `iphlpapi.dll` `SendARP`, cross-platform `LanScannerService` hosted-service scheduler with `PeriodicTimer` cadence, chain-event emission for `LanDeviceFirstSeen` / `LanDeviceMacChanged`, `ScannerOptions` for live-reload-able scan interval. mDNS + NetBIOS hostname resolution deferred to 9.2.5; RPCs to 9.3; UI to 9.4)
**Test count:** 943
