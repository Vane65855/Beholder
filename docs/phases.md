...
**Current checkpoint:** Phase 13.1 (Settings tab foundation — three read-only sections: Data Storage, Maintenance, About. New `GetStorageStats` RPC bundles per-table SQLite row counts + database file size + the cached chain-verification status into one fetch. New `IChainStatusCache` singleton in `Beholder.Core` shared between the periodic `ChainIntegrityMonitor` and the user-triggered `VerifyChain` RPC so the Settings tab's "Last verified" label surfaces the most-recent outcome regardless of which writer produced it. UI: `SettingsTabViewModel` mirrors `ScannerTabViewModel`'s cold-start-race + transient-banner + relative-time-ticker patterns; the "Verify chain integrity now" button on Maintenance flips the cache + surfaces success/failure in a transient banner; the "Open data folder" button uses a testable `IFolderOpener` seam wrapping `Process.Start`. Interactive sections — Storage retention preset, Recording, Hostname Resolution, Alerts, Scanner — deferred one-per-sub-phase to 13.2–13.6, each adding its own runtime-mutable state singleton following the `FirewallEnforcementState` precedent. RPC surface 18 → 19.)
**Test count:** 1187
