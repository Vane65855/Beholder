...
**Current checkpoint:** Phase 9.4 (Scanner tab UI — replaces the `ScannerTabViewModel` + `ScannerTabView` stubs with a real Avalonia master-detail surface that consumes the Phase 9.3 IPC: `ListLanDevicesAsync` for the initial snapshot, `LanDeviceFirstSeen` / `LanDeviceMacChanged` stream events for live updates, `TriggerScanAsync` behind a "Scan now" header button with the structured response surfaced as a transient warn / danger banner. Mirrors the `AlertsTabViewModel` precedent: cold-start race via `Task? _activationTask`, single-tab state ownership, all five required UI states implemented and tested. Status dot per row tints to "stale" after 10 min without an observation; relative-time labels refresh on a 1-second `IDispatcher`-marshaled ticker. End-to-end loop closed: daemon discovers → store persists → chain audits → broadcast pushes → UI renders.)
**Test count:** 1099
