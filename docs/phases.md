...
**Current checkpoint:** Phase 13.6 (Settings tab — Application Identity Overrides section. Closes [ADR 007](decisions/007-logical-app-identity-and-spoof-detection.md)'s explicit gap for unsigned binaries / binaries without usable VersionInfo: a manual fallback tier where the user picks the offending binary in Settings and confirms the **anchor folder** (the stable parent above the version-numbered subfolder), and from then on any binary at exactly one level below that anchor with the same filename is silenced on first-seen. Strict depth-1 match semantics — `Path.GetFileName(P) == filename AND Path.GetDirectoryName(Path.GetDirectoryName(P)) == anchor_path` — keep the rule from accidentally over-matching deeper folder structures. Slots in as **Tier 2.5** in `NewProcessDetector.ProcessAsync` between automatic signed identity (Tier 2 wins — cryptographic proof beats user override) and genuinely-new path (Tier 3 fires the alert). New SQLite `app_identity_rule` table with `UNIQUE(anchor_path, filename)` + `idx_app_identity_filename` index; new `IAppIdentityRuleStore` interface + `SqliteAppIdentityRuleStore` impl. Three new RPCs (`AddAppIdentityRule` with soft-failure on duplicate, `RemoveAppIdentityRule` idempotent, `ListAppIdentityRules`) take the surface 24 → 27. Two new `EventKind` ordinals (`AppIdentityRuleCreated = 15`, `AppIdentityRuleRemoved = 16`) + one `AppIdentityRulePayloadEncoder` handling both. New `IFilePicker` UI abstraction (prod `AvaloniaFilePicker` via `TopLevel.GetTopLevel(window).StorageProvider`, test `FakeFilePicker`) wired through `App.axaml.cs` using the existing `Func<MainWindow?>` capture-by-reference pattern from `AvaloniaClipboardWriter`. New "APPLICATION IDENTITY OVERRIDES" section card between Scanner and Maintenance with inline ADD card (file picker + auto-detected FILE/VARIABLE/ANCHOR segments + live "Will match" preview + inline validation error when the picked file isn't exactly one segment below the anchor + SAVE/CANCEL). Stupid-in/stupid-out: the daemon trusts the user's explicit rule; the only hard guard rail is the depth-1 validation check at ADD time in the UI. See [ADR 011](decisions/011-manual-app-identity-rules.md) for the full rationale. 1322 → 1360 tests pass deterministically (+38 across `SqliteAppIdentityRuleStoreTests`, `AppIdentityRulePayloadEncoderTests`, extensions to `NewProcessDetectorTests`, `SettingsTabViewModelTests`, plus new test doubles `FakeAppIdentityRuleStore` + `FakeFilePicker`). After 13.6, **five Settings sections actively mutate daemon state** (Recording, Hostname Resolution, Alerts, Scanner, Application Identity Overrides). Only 13.5 (Storage retention preset switcher) remains as a non-trivial Settings sub-phase, explicitly post-launch backlog.)

**Previous checkpoint:** Phase 9.6 (Scanner → Traffic cross-link. The Scanner tab's detail pane gains a "VIEW IN TRAFFIC" button that switches to the Traffic tab and filters its per-process list to only processes that exchanged data with the selected device's IP — closes the natural follow-up loop "what's on my LAN?" → "what traffic do I actually exchange with this device?". Reuses the Phase 7 Alerts → Firewall deep-link precedent (`INavigationService` delegate-via-constructor, await-activation-then-apply for cold-start safety). Extends the existing `GetProcessSummaries` RPC with an optional `remote_address` filter (proto3 empty-string default = no filter, preserving the pre-9.6 contract) + five new SQL indexes `idx_*_address_time` across the five traffic tier tables. `TrafficTabViewModel` gains `RemoteAddressFilter` observable + a "FILTERED TO IP: x.x.x.x [×]" chip; in live mode the filter auto-bumps the range to "1 Hour" so the SQL filter has historical data to bite on. The aggregate timeline chart stays unfiltered — the user sees the filtered process list in context of overall network activity. Bidirectional cross-link (Traffic → Scanner) and per-IP timeline chart in GRAPH view are explicitly deferred — the per-process list is the headline answer. RPC surface unchanged at 24. 1308 → 1322 tests pass deterministically (+14 across `SqliteTrafficStoreTests`, `ScannerTabViewModelTests`, `TrafficTabViewModelTests`). This is the last user-facing feature gap before launch.)

**Previous checkpoint:** Phase 13.4 (Settings tab — Scanner section. Third interactive Settings sub-phase; one toggle `EnableHostnameResolution`. **Plan deviation:** the original 13.4 plan declared this as "next daemon start" matching `EnablePreload`/`EnableSniCapture`, but implementation surfaced a construction-vs-StartAsync ordering bug — the DI factory for `ILanDeviceProbe` was reading `IScannerSettingsState` at construction time, BEFORE `SettingsOverridesService.StartAsync` applies persisted overrides. Two fixes were possible: defer the factory's read, or always inject both probes and gate at scan time via the state singleton. Picked the latter — simpler, removes the bug, AND lifts the "(takes effect on next daemon start)" caption to live UX. The cost is negligible (probes always instantiated, but only invoked when the toggle is on). Sets a useful precedent for future Settings sub-phases: when snapshot-at-construction produces override-ordering bugs, prefer per-call live reads over deferred-construction fixes. The two Phase 13.2 "next-start" toggles aren't affected — their consumers (`EtwDnsCache.PreloadFromWindowsDnsCache`, `PktmonSniSource.StartAsync`) already read at their own `StartAsync` time, which is correctly ordered after `SettingsOverridesService.StartAsync`. RPC surface 23 → 24. New `EventKind.ScannerSettingsChanged = 14` + `ScannerSettingsPayloadEncoder`. 1294 → 1308 tests pass deterministically (+14 across `ScannerSettingsStateTests`, `ScannerSettingsPayloadEncoderTests`, `SettingsRpcTests`, `SettingsTabViewModelTests.Phase132`). After 13.4, four Settings sections actively mutate daemon state (Recording, Hostname Resolution, Alerts, Scanner). Only 13.5 (Storage retention preset switcher) remains as a non-trivial settings sub-phase.)

**Previous checkpoint:** Phase 13.3 (Settings tab — Alerts section. Second interactive Settings sub-phase; reuses Phase 13.2's runtime-mutable-settings pattern verbatim for three master kill-switches on the alert detector pipeline: `EnableNewProcessDetection` (per-flow-event live read in `NewProcessDetector`), `EnableHashChangeDetection` (per-tick live read in `BinaryHashMonitor`), `EnableChainIntegrityMonitor` (per-tick live read in `ChainIntegrityMonitor`). All three live — no restart-required captions. Numeric thresholds stay JSON-only matching the Phase 13.2 precedent. Behavior change (security fix): the mandatory startup chain verify can no longer be skipped via the UI toggle — `ChainIntegrityMonitor.StartAsync` always runs the startup verify; the toggle now gates only the periodic loop. Previously a corrupted chain could be silently hidden by toggling the monitor off before restart. RPC surface 22 → 23. New `EventKind.AlertSettingsChanged = 13` + `AlertSettingsPayloadEncoder`. 1276 → 1294 tests pass deterministically (+18 across `AlertSettingsStateTests`, `AlertSettingsPayloadEncoderTests`, `SettingsRpcTests`, `SettingsTabViewModelTests.Phase132`, and one modified `ChainIntegrityMonitorTests` test enforcing the new mandatory-startup-verify contract). Next up: Phase 13.4 (Scanner section).)
