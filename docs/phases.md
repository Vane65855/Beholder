...
**Current checkpoint:** Phase 12.1 (Windows service installation — the daemon installs itself as an auto-start **LocalSystem** Windows service. `Beholder.Daemon.exe --install` / `--uninstall` / `--status` (verbs parsed by the pure `ServiceCommandLine`, dispatched before the host builds) shell `sc.exe` to register the service with auto-start + failure-recovery, and `icacls` to harden the data directory's ACLs. `builder.Host.UseWindowsService()` cooperates with the SCM lifetime and routes logs to the Windows Event Log — a no-op for a console `dotnet run`, so development is unaffected. A new `DaemonPaths` resolver places mutable data (SQLite db + Ed25519 keys) under `%ProgramData%\Beholder` when SCM-hosted and exe-relative in dev, finally realizing ADR 012's documented intent; read-only bundled assets (GeoIP MMDB, OUI csv) stay beside the binary. LocalSystem already holds the ETW + WFP privileges the daemon needs, so no app manifest is required. The icacls hardening is load-bearing: ProgramData's default ACL grants Users *read*, which would leave the Ed25519 signing key world-readable — stripping inheritance + granting only SYSTEM/Administrators makes ADR 012's key-security claim real. Removed the per-second `Worker` heartbeat (Event Log spam; the SCM reports liveness). **Security gap recorded in 12.1, since closed in [Phase 12.6](decisions/014-local-control-channel-named-pipe.md):** the control RPC was unauthenticated TCP `localhost:50051`, not the named-pipe-DACL design ARCHITECTURE.md describes — an always-on service widened that exposure; 12.6 moves it to a `beholder-users`-DACL'd named pipe. WiX/MSI packaging + Phase 12.5 firewall reconciliation remain deferred. See [ADR 013](decisions/013-windows-service-model.md). **1459 tests pass** (+1 skipped, Administrator-only ETW); +25 across `DaemonPathsTests`, `ServiceCommandLineTests`, and `WindowsServiceInstallerTests` (the pure sc/icacls argument builders — the elevated install itself is a manual smoke test, mirroring the ETW tests).)

**Previous checkpoint:** Phase 11.3 (signed chain export — an `ExportChain` RPC + Settings → Maintenance "EXPORT CHAIN" button emit a signed, self-verifying JSON envelope any third party can verify for authenticity and internal hash-chain consistency; completes Phase 11 (signed checkpoints + verify-from-anchor + signed export). RPC surface 27 → 28. See [ADR 012](decisions/012-signed-checkpoints-and-chain-export.md).)
**Test count:** 1459
...
### Phase 13.7 — Settings tab: Traffic Totals ("Exclude from totals") ✅

**Purpose:** VPN/tunnel clients double-count traffic in a per-process monitor — every app's bytes re-count through the tunnel binary, so a WireGuard machine shows ~2× its real WAN traffic in the All-processes totals, the aggregate chart is dominated by the tunnel echo, and the Map grows a blob in the VPN endpoint's country. 13.7 adds a user-managed exclusion list that removes such processes from every *aggregate* view while leaving recording, alerts, the chain, and per-process inspection completely untouched. Full rationale in [ADR 016](decisions/016-exclude-from-totals.md).

**Decision (aggregate-READ-time, never capture-time).** The tempting reuse of `Recording.FilterSelfTraffic`'s capture-time drop would make the one binary all traffic transits invisible to `NewProcess`/`HashChanged` alerting — a self-inflicted blind spot. Instead the daemon enforces exclusion only in all-processes aggregate SQL (`GetAggregateTimeline` via `TimelineStitcher` incl. its data-extent scan; destinations/country/protocol breakdowns when `process_path` is null) through one shared `ProcessExclusionSqlFilter` (`process_path COLLATE NOCASE NOT IN (...)`; empty list → empty clause). **Explicit selection always wins** — per-process views include an excluded process, and `GetProcessSummaries` keeps its rows so the UI decides presentation.

**Decision (ADR 010 section pattern, list-valued).** `ITotalsExclusionState`/`TotalsExclusionState` (lock-protected `OrdinalIgnoreCase` set, seeds empty); persistence = one `settings_overrides` JSON string-array row (`Traffic.ExcludedProcessPaths`; `SettingsOverridesService` gains `ReadStringListOverride`); whole-list `SetTotalsSettings` RPC (normalize: trim/drop-empties/dedupe; surface 28 → 29) + `GetSettings` gains the `totals` bundle; `EventKind.TotalsExclusionsChanged = 17` whose payload carries the **full post-change list** — hiding a process from totals is itself always chain-audited. Matching is case-insensitive end to end (picker paths are filesystem-cased, recorded paths come from ETW).

**Decision (hidden by default; show-toggle is a UI preference; UI mirrors the list).** Excluded rows disappear from the Traffic tab's list; a "Show excluded processes" toggle re-lists them with a ⊘ marker (never counted either way). The toggle is display-only → `JsonUiPreferencesStore.ShowExcludedProcesses` (close-to-tray precedent). Live client-side aggregation (status strip, All chart, all-row) consults a `TotalsExclusionUiState` mirror seeded from `GetSettings` on every connect and updated with each echoed Set response — chosen over stamping an `excluded_from_totals` proto flag per snapshot because this single-instance UI is the list's only writer and the flag would cost a proto field + ~43 `BroadcastService` ctor call-sites for no behavioral difference (see ADR 016). Exclusion changes re-render the Traffic tab live and re-query historical ranges/COLS/MAP.

**Changes:**
- **Core (new):** `ITotalsExclusionState` + `TotalsExclusionSnapshot`; `SettingsKeys.TrafficExcludedProcessPaths`; `EventKind.TotalsExclusionsChanged = 17`; `DestinationsQuery.ExcludedProcessPaths` init property; optional `excludedProcessPaths` parameter on `ITrafficStore`'s three aggregate queries.
- **Daemon (new):** `TotalsExclusionState`; `TotalsExclusionsPayloadEncoder`; `ProcessExclusionSqlFilter`.
- **Daemon (modified):** `SqliteTrafficStore` + `TimelineStitcher` apply the exclusion clause in all-processes mode; `BeholderLocalService` gains the `SetTotalsSettings` handler (+ ctor param, GetSettings packs totals, aggregate handlers pass the state's list); `SettingsOverridesService` replays the persisted list; `Program.cs` registers the singleton.
- **Protocol:** `TotalsSettingsValues` + `SetTotalsSettingsRequest/Response` + `GetSettingsResponse.totals` + the RPC; `ProtocolConverters` ToProto/ToDomain for the snapshot.
- **UI (new):** `TotalsExclusionUiState` (mirror + `Changed` event + connect-time `RefreshFromDaemonAsync`); `TotalsSettingsRow`; `UiPreferences.ShowExcludedProcesses`.
- **UI (modified):** `ProcessListCoordinator` (ctor takes the mirror; hide-or-mark logic; all-row math skips excluded; `IsExcludedProcess` renamed `IsPseudoProcess` to avoid semantic clash); `ProcessListItem.IsExcludedFromTotals` drives a ⊘ marker in `TrafficTabView` (header + row template gain a marker column); `StatusStripViewModel` + `TrafficTabViewModel` aggregation loops skip excluded (`AggregateAllInto`/`ComputeMaxLength` de-static'd); `TrafficTabViewModel` reacts to `Changed` (live re-render, historical/COLS/MAP re-query, chart selection cleared); `SettingsTabViewModel` gains the TRAFFIC TOTALS card commands (add via `IFilePicker`, per-row remove, show-toggle) with whole-list save + echo-apply; `IDaemonClient`/`DaemonClient` gain `SetTotalsSettingsAsync`; `App.axaml.cs` constructs + seeds the mirror and chains it into `OnConnected` before the process-state seed.
- **appsettings.json:** no change — the list has no meaningful file default (seeds empty; overrides come from `settings_overrides`).

**Tests (1360 → 1576 across the three commits, +51 for 13.7 itself):** `TotalsExclusionStateTests` (seed/idempotence/case-insensitive/event contract), `TotalsExclusionsPayloadEncoderTests` (round-trip, malformed → null), `SettingsRpcTests` (+6: totals bundle in Get; Set persists JSON array + chains once; normalization; no-op; InvalidArgument; persistence soft-failure), `SettingsOverridesServiceTests` (+2: list replay, malformed stays empty), `SqliteTrafficStoreTests` (+9: aggregate excludes / NOCASE / all-excluded → empty / empty-list no-op / destinations-country-protocol gate on explicit process / summaries keep rows), `GetAggregateTimelineRpcTests` (+1 end-to-end through the handler), `TotalsExclusionUiStateTests` (5), `ProcessListCoordinatorTests` (+4 hide/mark/late-exclusion/historical), `StatusStripViewModelTests` (+1 every-figure skip), `TrafficTabViewModelTests` (+2 All-chart skip, live re-render on change), `SettingsTabViewModelTests` (+7 card commands + activate sync). Bulk ctor updates via the established compiler-driven pattern (`BeholderLocalService` 22 sites, `SettingsOverridesService` 5, UI VMs 20+).

**Docs:** new [ADR 016](decisions/016-exclude-from-totals.md); ARCHITECTURE.md IPC listing + Recording Policy cross-note; this entry.

**Build clean (0 warnings). 1576 tests pass deterministically 3× (1575 passing + 1 skipped Administrator-only ETW test). Runtime-verified against a live daemon: excluding the VPN drops the strip totals/chart to real WAN volume, the row hides, and the show-toggle re-lists it with the ⊘ marker.**

---

### Phase 13.8 — Wall-clock-true traffic charts (selection-window fix) ✅

**Purpose:** A user-reported bug — a visibly selected bump on the live chart returned "0 destinations" — traced to one assumption failing in three places: *the chart's pixel axis is linear in wall-clock time*. It wasn't: the daemon skipped broadcasts on idle seconds (live buffers compressed time, so the selection's fraction→wall-clock mapping drifted ~13 s and missed a 14 s window whose traffic provably sat in the DB), the stitched historical timeline omitted empty buckets (gaps compressed the axis, and the selection mapped fractions onto the *requested* range — on All Time that starts at the Unix epoch, so every selection landed decades before the data), and `TierSelector.Select` ignored retention (a 30-minute selection window older than 10 minutes routed to `traffic_raw` → guaranteed empty). Full rationale in [ADR 017](decisions/017-wall-clock-true-charts.md). Five atomic commits:

1. **`daemon/traffic` — heartbeat batches.** `TrafficEngine.OnSnapshotBatch` fires every tick, empty when idle; the empty batch is the UI's per-second clock (its existing absent-process loop zero-fills every buffer). Wire-compatible both directions; ~30 B/s per subscriber only while a UI is attached. Side effect: the live chart scrolls and rates decay to 0 during idle instead of freezing. Also hardened `TrafficEngineTests.DriveTickAsync` (wait signal installed before the event writes — closes a rare full-suite-load timeout).
2. **`daemon/storage` — zero-filled stitched timelines.** `TimelineStitcher` expands the merged result onto the contiguous uniform grid (explicit zero points, bounded ≤ ~400 buckets per extent by the nice-resolution pick). Idle stretches render flat instead of compressed away; index↔wall-clock linear; "same data → same chart" preserved; empty stays empty. Kills the rejected alternative of shipping `effective_resolution_ms` over the proto — the UI infers the step from the endpoints.
3. **`daemon/storage` — retention-aware tier selection.** `Select` only considers tiers whose retention covers `from`; coarsest-fitting-resolution among those, else finest covering (rollup invariant makes summary sums identical wherever tiers overlap, so completeness is free). Every preset resolves to the same tier as before — the delta is exactly small windows anywhere in history.
4. **`ui/traffic` — wall-clock-true live buffers.** `ProcessStateService` gap-fills zero samples from `tick_timestamp_unix_ns` deltas before applying each batch (covers DropOldest drops, OS sleep, old daemons; inert on zero/unknown timestamps; capped at the 300-sample window) and the seed pads trailing zeros from the last timeline bucket to *now* + sets the tick baseline to the seed instant.
5. **`ui/traffic` — selection maps onto the drawn domain.** `ApplyHistoricalChart` captures the drawn domain (first/last sample + grid step, single-point spike layout included); `OnChartSelectionChanged` maps fractions through it in historical mode (live keeps the now-exact resolved window), aligns the fetch window outward to the drawn bucket grid (floor start, extend end past its bucket — a band edge on a drawn peak includes that bucket), labels the aligned window, and drops gestures on empty historical charts.

**Tests (1590 → 1608, +18 net):** `TrafficEngineTests` (idle tick → empty heartbeat batch), `BroadcastServiceTests` (empty batch fans out with tick timestamp), `SqliteTrafficStoreTests` (gap zero-fill; grid assertions on the stitch/groups tests; small-old-window destinations served from `_10s`), `TierSelectionTests` (three ADR 017 window cases + three retention-aware expectation updates), `ProcessStateServiceTests` (+7: gap-fill values/absent processes/first-batch/backwards-clock/cap; seed trailing pad; seed baseline no-double-fill), `TrafficTabViewModelTests` (+5: drawn-domain mapping on All Time, outward bucket alignment, live whole-second alignment, single-point safety, empty-chart gesture drop).

**Docs:** new [ADR 017](decisions/017-wall-clock-true-charts.md); ARCHITECTURE.md (hot-path heartbeat note, single-tier selection rewrite); this entry.

**Build clean (0 warnings). 1608 tests pass deterministically 3× per commit (1607 passing + 1 skipped Administrator-only ETW test). Runtime-verified against the installed 0.1.231 daemon + UI: selecting a visible bump on the chart now lists the destinations the band contains — the originally-reported scenario is fixed.**
...
### Phase 12.1 — Windows service installation ✅

**Purpose:** Make the daemon an installable, auto-start Windows service instead of a console process launched by hand. Through Phase 11 it ran only via `app.Run()` from an elevated prompt; the `Microsoft.Extensions.Hosting.WindowsServices` package was referenced but unused. This also closes a documented-but-unimplemented gap: [ADR 012](decisions/012-signed-checkpoints-and-chain-export.md) said the service's data + keys live in `ProgramData\Beholder`, but every path still hung off `AppContext.BaseDirectory`. Full rationale in [ADR 013](decisions/013-windows-service-model.md).

**Decision (hosting + account).** `builder.Host.UseWindowsService(o => o.ServiceName = "Beholder")` — a no-op for a console run; under the SCM it installs the service lifetime + Windows Event Log logging. The service runs as **LocalSystem**, which already holds the ETW (NT Kernel Logger) + WFP privileges the daemon needs, so no `app.manifest` / `requestedExecutionLevel` is added.

**Decision (data directory).** New `DaemonPaths` resolver: `WritableDataRoot` = `%ProgramData%\Beholder` when `WindowsServiceHelpers.IsWindowsService()`, else `<exe>\data` (dev `dotnet run` stays local + unelevated); `ReadOnlyAssetRoot` = always `<exe>\data` for the bundled GeoIP MMDB + OUI csv (shipped read-only beside the binary). Only the SQLite db + Ed25519 keys relocate.

**Decision (key-dir hardening — load-bearing).** ProgramData's default ACL grants `Users` read, and a child folder inherits it — which would leave the private signing key world-readable and let any local user forge checkpoints. So `--install` runs `icacls <dir> /inheritance:r /grant:r *S-1-5-18:(OI)(CI)F *S-1-5-32-544:(OI)(CI)F` (strip inheritance; grant only LocalSystem + Administrators via locale-independent well-known SIDs), making ADR 012's "SYSTEM + Administrators" claim true rather than aspirational.

**Decision (install mechanism).** In-daemon CLI: `ServiceCommandLine.Parse(args)` (pure) maps `--install` / `--uninstall` / `--status`; `Program.cs` dispatches to `WindowsServiceInstaller` (in `Beholder.Daemon.Windows` per ADR 008) before building the host and returns its exit code. The installer shells the in-box `sc.exe` + `icacls.exe` resolved explicitly from `System32` (never via `PATH`), with `binPath= <Environment.ProcessPath>`, `start= auto`, a description, and failure-recovery. Elevation is checked up front (`WindowsPrincipal.IsInRole(Administrator)`) → clear message + exit 5 if missing. WiX/MSI was rejected for now (separate toolchain + code-signing, no pre-release benefit) — deferred to a packaging pass.

**Changes:**
- **Daemon (new):** `DaemonPaths` (path resolver); `ServiceCommandLine` + `ServiceCommand` enum (arg parsing).
- **Daemon (modified):** `Program.cs` — top-of-main service-control dispatch; `builder.Host.UseWindowsService(...)`; four `AppContext.BaseDirectory` path sites swapped to `DaemonPaths`; removed the `Worker` registration; explicit terminal `return 0` (the early `return`s make the entry point int-returning). **Deleted `Worker.cs`** (per-second heartbeat → Event Log spam; the SCM reports liveness).
- **Daemon.Windows (new):** `WindowsServiceInstaller` — `Install` / `Uninstall` / `Status` + pure sc/icacls argument builders.

**Tests (+25):**
- `DaemonPathsTests` (5) — resolver returns exe-relative when not-service / ProgramData when service; asset root always exe-relative; service ≠ dev root; `ServiceDataRoot` is `%ProgramData%\Beholder`.
- `ServiceCommandLineTests` (~12) — `--install` / `install` / `--INSTALL` → Install; uninstall / status / console / run / empty / unknown mappings; first-recognized-verb-wins; null throws.
- `WindowsServiceInstallerTests` (8, `#if PLATFORM_WINDOWS`) — the pure sc/icacls arg builders (binPath/start adjacency, failure-recovery, the harden-ACL SIDs grant SYSTEM+Admins and *not* Users/Everyone, the verb+name builders). The elevated install itself is a manual smoke test (mirrors the `[SKIP] Requires Administrator` ETW test).

**Build clean. 1459 tests pass deterministically (was 1434; +25), +1 skipped. The elevated install/uninstall + ACL hardening are verified by manual smoke test (admin + a real machine), not CI — like the ETW tests. Out of scope, flagged in ADR 013: the unauthenticated TCP-localhost control surface (the most important follow-up), WiX/MSI packaging, and Phase 12.5 firewall reconciliation.**

### Phase 12.6 — Local IPC hardening (named pipe + `beholder-users` DACL) ✅

**Purpose:** Close the top security gap flagged by [ADR 013](decisions/013-windows-service-model.md) — the daemon's gRPC control channel listened on unauthenticated TCP `127.0.0.1:50051`, reachable by any local process as any user, and making the daemon an always-on service widened that exposure.

**What shipped:**
- **Transport.** `Program.cs` swaps Kestrel's `ListenLocalhost(50051, Http2)` for `UseNamedPipes(…)` + `ListenNamedPipe(IpcEndpoint.PipeName, Http2)` — gRPC over `\\.\pipe\beholder`, still h2c, **no TCP fallback**. The pipe name is a single shared constant, `Beholder.Protocol.IpcEndpoint.PipeName`.
- **DACL (the boundary).** New `BeholderPipeSecurity` (Daemon.Windows) builds the pipe's `PipeSecurity`: SYSTEM + Administrators → FullControl; `beholder-users` → connect (ReadWrite|Synchronize); everyone else denied. Pure builder, unit-tested to grant exactly those three principals and **no** broad ones (Everyone `S-1-1-0` / Users `S-1-5-32-545` / Authenticated Users `S-1-5-11`). When the group can't be resolved (dev `dotnet run`, or pre-install) it falls back to INTERACTIVE and logs the downgrade.
- **Installer.** `--install` creates the `beholder-users` local group and adds the installing user via `net.exe localgroup` (pure arg builders, best-effort/idempotent); the install output tells the user to sign out/in (token-baked membership) and how to add others.
- **UI client.** `DaemonClient` (Windows) connects via a `SocketsHttpHandler.ConnectCallback` opening a `NamedPipeClientStream`; the reconnect/backoff/health-probe loop is unchanged. TCP is kept behind `#else` for a future Linux UDS.

See [ADR 014](decisions/014-local-control-channel-named-pipe.md). New unit tests: `BeholderPipeSecurityTests` (DACL grants/denials) + the `WindowsServiceInstaller` group arg-builders; the RPC tests are transport-agnostic (in-process `BeholderLocalService`) so they're unaffected. **Full suite green 3×** — the live pipe + elevated group creation are manual smoke tests, like the ETW work.

### Phase 12.7 — MSI installer (self-contained WiX v5) ✅

**Purpose:** Ship the product as a single double-click `.msi` instead of a dev tree + manual `--install`: install the daemon + UI to `C:\Program Files\Beholder`, register the service, and set up shortcuts with no prerequisites. The deferred packaging pass [ADR 013](decisions/013-windows-service-model.md) anticipated.

**What shipped:**
- **`Beholder.Installer/`** (`Beholder.Installer.wixproj` + `Package.wxs`, `WixToolset.Sdk/5.0.2`): a single self-contained win-x64 MSI. WiX **v5** because v7 hard-requires the paid OSMF EULA (WIX7015); v5 is free and still has the `<Files>` directory harvest. Both apps publish `--self-contained` into subfolders `Daemon\` + `UI\` (no framework-DLL collision). ~93 MB, no .NET prerequisite.
- **Service setup via the daemon's own verbs.** Deferred, non-impersonated custom actions run `Beholder.Daemon.exe --install` / `--uninstall` (which register the service + create `beholder-users` + harden the ProgramData ACL — none of which WiX has a primitive for), with a rollback action on install failure. One source of truth (DRY).
- **Shortcuts + auto-start.** Start Menu + Desktop open the UI; a per-user Startup shortcut passes `--tray`. New `Beholder.Ui/StartupOptions.cs` + an `App.axaml.cs` branch start the window hidden in the tray on `--tray`/`--minimized` (Avalonia otherwise auto-shows it).
- **Groundwork.** Root `Directory.Build.props` (`<Version>0.1.0</Version>` — single-sourced into the assembly + MSI; fixes the UI About "0.0.0.0 / (dev build)"), `License.rtf` (AGPL for the WiX license dialog), `build-installer.ps1` (publish → strip PDBs → harvest), `.config/dotnet-tools.json` (pins WiX v5).

See [ADR 015](decisions/015-msi-packaging.md). The wixproj is intentionally **not** in `Beholder.slnx` (it would make every solution build emit a ~93 MB MSI). New tests: `StartupOptionsTests` (the `--tray` arg predicate). **Full suite green 3× (1491).** The MSI is structurally validated (WiX link + `wix msi decompile` checks: 751 files, both exes, the GeoIP/OUI data, 3 shortcuts, the 6 service custom actions); the elevated install/uninstall is a manual smoke test, like the sc/icacls/ETW work. The unsigned-SmartScreen and sign-out/in-to-connect caveats are real and documented.
...
- **`ProgramData` inheritance is insecure for secrets — set an explicit ACL.** The tempting "create the folder under `%ProgramData%` and let it inherit" leaves the daemon's Ed25519 *private* signing key readable by the `Users` group, because that's what ProgramData's default ACL grants. For a key that is the root of trust for the whole tamper-evident chain, that's a real hole (any local user could read it and forge checkpoints). The installer must actively `icacls /inheritance:r` + grant only SYSTEM + Administrators. A documented intent ("ACLs restrict to SYSTEM + Administrators") is not self-fulfilling — verify the *actual* inherited ACL, don't assume it.
- **One `return <value>` makes a whole top-level program int-returning.** Adding the `--install` early returns turned the entry point into `int Main`, and the compiler then demands every path return — including the normal `app.Run()` path, which needed an explicit terminal `return 0;` (CS0161 otherwise). The implicit "fall off the end → 0" only applies when *no* path returns a value.
- **`UseWindowsService()` / `IsWindowsService()` give a clean dev-vs-service split for free.** `builder.Host.UseWindowsService()` is a no-op when not SCM-hosted, so it's safe to call unconditionally and dev `dotnet run` is unaffected — while it's also what routes logs to the Event Log under the service. Pairing it with `WindowsServiceHelpers.IsWindowsService()` for path resolution means the *same binary* writes data exe-relative in development and to `%ProgramData%` as a service, with no config flag or build variant.
- **Build sc.exe arguments as a list, not a hand-quoted string.** `sc create … binPath= "C:\Program Files\…\x.exe"` is a notorious quoting trap. Passing each token via `ProcessStartInfo.ArgumentList` (so `binPath=` and the path are *separate* entries) lets the runtime quote the spaced path correctly and makes the argument vector a pure, unit-testable value — the elevated execution stays a manual smoke test, but the arg-building is covered.
...
- **Unauthenticated local control RPC (security — raised in priority by Phase 12.1).** The daemon's control surface listens on TCP `127.0.0.1:50051` with no authentication, while [ARCHITECTURE.md](ARCHITECTURE.md) / [CLAUDE.md](../CLAUDE.md) describe a named pipe DACL'd to the local user / a `beholder-users` group. Any local process — any user — can currently connect and issue firewall commands. Phase 12.1 made the daemon an always-on auto-start service, which **widens this exposure window**, so closing it is now the top hardening item: switch to a DACL'd named pipe (matching the documented design) or add local-caller auth, and reconcile the docs to whatever ships. Recorded in [ADR 013](decisions/013-windows-service-model.md); its own pass, not folded into 12.1.
- **Startup OS/SQLite firewall reconciliation** — on daemon restart, OS firewall rules and SQLite `firewall_rules` table may diverge (crash during apply, manual OS changes). A reconciliation pass at startup is deferred to Phase 12 (12.5).
