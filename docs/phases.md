...
**Current checkpoint:** Phase 9.5 (Scanner — manual device labels + Router DNS reverse-lookup probe. Two related hostname-quality features that close the gap to GlassWire's "Things" tab UX. Storage: `lan_device` table gains a nullable `label` column with an idempotent `MigrateLanDeviceFor95` migration; the column is deliberately omitted from `UpsertAsync`'s `ON CONFLICT` clause so user labels survive scanner re-observations. IPC: new `SetLanDeviceLabel` RPC (RPC surface 17 → 18) + new `LanDeviceLabelChangedEvent` oneof variant (5 → 6 stream variants). Probe: `RouterDnsHostnameProbe : IHostnameProbe` reuses ADR 005's `IReverseDnsResolver` to call `Dns.GetHostEntryAsync` against the OS resolver, picking up DHCP-table hostnames from routers that mirror DHCP into LAN DNS — appended after mDNS-PTR + NetBIOS in the per-IP ladder. UI: Scanner-tab detail pane gains an editable CUSTOM NAME row at the top (RENAME / SAVE / CANCEL); master-list rows promote to `LabelOrFallback` (label > hostname > IP) so renaming a device flips its master-list primary text instantly via the live broadcast. Labels NOT chain-audited (cosmetic UI state, same category as Alert read-state). RouterDnsHostnameProbe hit-rate is router-dependent — many home routers publish DHCP hostnames, HUAWEI / some budget models don't; the manual-label UX is the long-tail backstop.)
**Test count:** 1130
...
- **Manual device labels** (right-click → "Rename") — Phase 9.5. Web research confirms GlassWire's primary UX for human-friendly device names is exactly this; deferred to follow Beholder's discovery-first prioritization.
...
- **DHCP option 12 sniffing** on UDP/68 — Phase 9.2.7 candidate. Web research confirmed neither GlassWire nor any consumer tool sniffs DHCP for hostnames; this would be a true differentiator. Defer.
...
- **A web-research pass to confirm what competitors actually do beats assuming.** During 9.4 smoke testing, the user observed that their Samsung S25 had a "Send Device Name to Network" toggle (which sends the hostname via DHCP option 12) and asked whether Beholder could pick it up. The initial answer was "yes, but it'd need a DHCP packet sniffer in Phase 9.2.7." A second pass to verify "how does GlassWire handle this?" revealed that GlassWire doesn't sniff DHCP either — they fall back to OUI vendor name + a user-facing manual-label UX. That research reframed the roadmap: instead of a single big DHCP-sniffer subphase, two cheaper wins (router DNS reverse-lookup + manual device labels) match GlassWire on the common case, and DHCP sniffing becomes a true differentiator. The lesson: when a feature gap surfaces during user testing, do a 15-minute research pass on prior art before committing the next subphase — the "obvious" implementation is sometimes overkill, and the cheap path that matches the market leader is the better next move.
