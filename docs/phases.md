...
**11.3 — Signed chain export.** New `ExportChain` RPC returns a signed, self-verifying JSON envelope of the chain (optional `from_seq`/`to_seq` range; `(0,0)` = whole chain), saved to a user-chosen path by a Settings → Maintenance "EXPORT CHAIN" button. The envelope is `{ body, signature_alg, key_id, public_key_b64, signature_b64 }`; the signature is a **detached signature over a digest of the canonical body** — `Ed25519.Sign(SHA-256(canonicalUtf8(body)))` — with canonical form pinned by one `static readonly JsonSerializerOptions` + `[JsonPropertyOrder]` on every envelope record (the JWS-detached-sig / Sigstore approach, avoiding serializer-ordering fragility). This honors the user's "sign the entire envelope, events included" choice. A receiver verifies **two independent guarantees** with stock crypto and no Beholder install: authenticity (re-canonicalize `body`, SHA-256, verify against `public_key_b64`) and internal consistency (recompute each `row_hash` against the embedded chain). `ChainExporter.TryVerify` is the reference routine ADR 012 documents for third parties.
...
**Build clean. 1432 tests pass (+1 skipped Administrator-only ETW test). The Phase 11.3 suite (102 tests across the four classes above) is green deterministically 3×. One pre-existing wall-clock flake unrelated to Phase 11 — `ArpScanProbeTests.ProbeIpsAsync_ParallelDispatch_RunsConcurrently`, a `Thread.Sleep`-based parallelism assertion with a tight 1.5 s threshold — intermittently trips under full-suite thread-pool contention and passes in isolation (16/16). Phase 11 is complete: the §5 O(n)-verify gap is closed and the chain is now externally attestable.**
...
- **A tight wall-clock parallelism assertion is a latent flake under full-suite load.** `ArpScanProbeTests.ProbeIpsAsync_ParallelDispatch_RunsConcurrently` (`Thread.Sleep(100)` × 64 IPs, assert `< 1.5 s`) passes 16/16 in isolation but trips intermittently in the full run, where thread-pool injection latency under contention pushes the blocking sleeps past the threshold. Surfaced during 11.3 verification; pre-existing, not a Phase 11 regression. The principled fix (deferred to Phase 12 hardening, out of 11.3 scope) is to assert against the *sequential* baseline (6.4 s) with generous headroom rather than a hard 1.5 s — the test's intent is "concurrent, not serial," which a `< 3 s` bound proves without racing the scheduler.
