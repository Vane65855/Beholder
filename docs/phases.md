...
**Current checkpoint:** Phase 9.2.5 (Scanner hostname resolution — mDNS via raw-UDP multicast PTR queries to 224.0.0.251:5353 with the QU bit per RFC 6762 §5.4, plus NetBIOS NBSTAT unicast queries to port 137 per RFC 1002 §4.2.17, orchestrated per-IP by a parallel `HostnameResolutionLadder`. Completes ADR 009's three-layer hostname-resolution design. No new P/Invoke — pure managed `UdpClient` + bytes-on-wire parsers mirroring the `TlsClientHelloParser` precedent from ADR 006)
**Test count:** 996
